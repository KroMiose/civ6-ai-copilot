...
const requiredBundleFiles = [
  ...
  "tooling/tools/copilot/src/context-cli.ts",
  "tooling/tools/copilot/src/context.ts",
...
async function writeReleaseChecklist(bundleDir: string): Promise<void> {
  const checklist = [
    ...
    "5. Open the Copilot panel and click `更新战情`; it refreshes all implemented player-visible modules including the budgeted visible map.",
   ...
   "   ```",
    ...
    "",
    "2. Normal same-machine Agent analysis uses the canonical context runtime:",
    "",
    "   ```bash",
    "   cd tooling",
    "   npm run context -- --query \"<question>\"",
    "   ```",
    "",
    "   Cross-device compatibility can still sync the Windows `<snapshot-dir>` or generate a handoff directory:",
    "",
    "   ```bash",
    "   cd tooling",
