...
When upgrading from `0.1.x` to the 0.3 protocol, update the Mod, Skill, and local tools together. Old snapshots cannot be used with the 0.3 protocol. `0.3.7` adds levy, readable session info, and a world index, so install the Mod from this release. Replace the whole skill folder as well. Steam Workshop is not updated in this release.
...
Current pre-release version: `0.3.7`.
