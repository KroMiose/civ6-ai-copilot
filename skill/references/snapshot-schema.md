...
`visibleMap` 使用 odd-r offset，y 更大在屏幕上方。奇数 y 行相对偶数 y 行向右错半格。相邻六格：
...
日常移动和选址优先使用 `context --adjacent-units` 返回的相邻格，不必自己试坐标。
...
1. 运行 `node scripts/context.mjs` 读取决策简报。不要把玩家原话转成内部意图，也不要在日常分析里读取 handoff。
2. 用简报里的 `gaps` 判断哪些结论必须降级。
3. 铺城、移动和战线再运行 `--map`。若需要一座城或一个单位的建筑、产出或晋升，再运行 `--city` 或 `--unit`。
