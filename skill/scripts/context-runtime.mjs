...
// tools/copilot/src/context.ts
import { createHash as createHash3 } from "node:crypto";
import { mkdir as mkdir4, readFile as readFile4, writeFile as writeFile4 } from "node:fs/promises";
import path5 from "node:path";

// tools/paths/src/civ6-paths.ts
import os from "node:os";
import path from "node:path";
import { existsSync } from "node:fs";
function buildCiv6AICopilotPaths(options2 = {}) {
  const platform = normalizePlatform(options2.platform ?? os.platform());
  const pathApi = platform === "win32" ? path.win32 : path.posix;
  const homeDir = options2.homeDir ?? defaultHomeDir(platform);
  const documentsDir = pathApi.join(homeDir, "Documents");
  const civ6UserDataDir = options2.civ6UserDataDir ?? defaultCiv6UserDataDir(platform, homeDir);
  const modsDir = options2.modsDir ?? defaultCiv6ModsDir(platform, civ6UserDataDir);
  const logsDir = options2.logsDir ?? defaultCiv6LogsDir(platform, civ6UserDataDir, homeDir);
  const luaLogPath = options2.luaLogPath ?? pathApi.join(logsDir, "Lua.log");
  ...
  const snapshotDir = options2.snapshotDir ?? pathApi.join(documentsDir, "civ6-ai-copilot-snapshots");
  const handoffDir = options2.handoffDir ?? pathApi.join(documentsDir, "civ6-ai-copilot-handoff");
  const codexHome = options2.codexHome ?? pathApi.join(homeDir, ".codex");
  const skillInstallDir = pathApi.join(codexHome, "skills", "civ6-ai-copilot");
  const analysisArgs = buildAnalysisArgs(options2);
...
function buildAnalysisArgs(options2) {
  const intents = normalizedList(options2.intents);
  const modules = normalizedList(options2.requiredModules);
...
function defaultCiv6UserDataDir(platform, homeDir) {
  if (platform === "win32") {
...
function defaultCiv6ModsDir(platform, civ6UserDataDir) {
  if (platform === "win32") {
...
function defaultCiv6LogsDir(platform, civ6UserDataDir, homeDir) {
  if (platform === "win32") {
...
// tools/render-map/src/render-map.ts
import { readFile, writeFile } from "node:fs/promises";

// tools/snapshot/src/validate.ts
var import__ = __toESM(require__(), 1);
var import_ajv_formats = __toESM(require_dist(), 1);

// schemas/snapshot.schema.json
var snapshot_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://civ6-ai-copilot.local/schemas/snapshot.schema.json",
  title: "civ6-ai-copilot player-visible snapshot",
  type: "object",
  required: [
    "schemaVersion",
    "exportedAt",
  ...
  properties: {
    ...
    source: {
      type: "object",
      required: ["modId", "modVersion", "compatVersion", "transport", "visibilityMode", "protocolVersion", "exportId"],
      additionalProperties: false,
      properties: {
        modId: { type: "string", const: "civ6-ai-copilot" },
        modVersion: { type: "string" },
        compatVersion: { type: "string", pattern: "^\\d+\\.\\d+$" },
        transport: { type: "string", enum: ["lua-log"] },
        visibilityMode: { type: "string", const: "player-visible" },
        exportId: { type: "string" },
        exportType: { type: "string", enum: ["turn", "full", "modules", "map-window", "visible-map", "diagnostic"] },
        protocolVersion: { type: "string", const: "0.3.0" }
      }
    },
    session: {
      type: "object",
      required: ["sessionId", "gameTurn", "ruleset", "gameSpeed", "mapSize", "isMultiplayer", "idScope"],
      additionalProperties: false,
      properties: {
        sessionId: { type: "string", minLength: 1 },
        gameTurn: { type: "integer", minimum: 0 },
        ruleset: { type: "string" },
    ...
    localPlayer: { $ref: "#/$defs/playerSummary" },
    modules: {
      type: "array",
      items: { type: "string", enum: ["meta", "localPlayer", "selection", "cities", "units", "governors", "trade", "cityStates", "techs", "civics", "government", "policies", "resources", "diplomacyPublic", "visibleMap", "economy"] },
      uniqueItems: true
    },
    ...
    governors: { $ref: "#/$defs/governors" },
    trade: { $ref: "#/$defs/trade" },
    cityStates: { $ref: "#/$defs/cityStates" },
    visibleMap: {
      type: "object",
      required: ["source", "visibility", "confidence", "tiles"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
        confidence: { $ref: "#/$defs/confidenceKind" },
        scope: { type: "string", enum: ["player-visible-revealed"] },
    ...
    techs: { $ref: "#/$defs/progression" },
    civics: { $ref: "#/$defs/progression" },
    government: {
      type: "object",
      required: ["source", "visibility", "confidence", "currentGovernment", "policySlots", "policies"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
        confidence: { $ref: "#/$defs/confidenceKind" },
        currentGovernment: { $ref: "#/$defs/namedType" },
        policySlots: {
          type: "object",
        ...
        canChangePolicies: { type: "boolean" },
        availablePolicies: {
          type: "array",
          items: {
            type: "object",
    ...
    resources: {
      type: "object",
      required: ["source", "visibility", "confidence", "items"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
    ...
    diplomacy: {
      type: "object",
      required: ["source", "visibility", "confidence", "metPlayers"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
        ...
        metPlayers: {
          ...
          items: {
            type: "object",
            required: ["playerId", "civilizationType", "relationship"],
            additionalProperties: false,
            properties: {
              playerId: { type: "integer" },
              civilizationType: { type: "string" },
              leaderType: { type: "string" },
              relationship: { type: "string" },
              militaryScore: { type: "number" },
              visibility: { $ref: "#/$defs/visibilityKind" },
              source: { $ref: "#/$defs/sourceKind" },
              confidence: { $ref: "#/$defs/confidenceKind" }
            }
    ...
    attention: {
      ...
      items: {
        type: "object",
        required: ["kind", "message", "severity", "source", "visibility", "confidence"],
        additionalProperties: false,
        properties: {
          kind: { type: "string" },
          message: { type: "string" },
    ...
    confidence: {
      type: "object",
      required: ["overall"],
      additionalProperties: { $ref: "#/$defs/confidenceKind" },
      properties: {
        overall: { $ref: "#/$defs/confidenceKind" }
      }
    },
    extensions: { type: "object", additionalProperties: true },
    moduleStatus: {
      type: "object",
      propertyNames: { enum: ["meta", "localPlayer", "selection", "cities", "units", "governors", "trade", "cityStates", "techs", "civics", "government", "policies", "resources", "diplomacyPublic", "visibleMap", "economy"] },
      additionalProperties: {
        type: "object",
        additionalProperties: false,
        required: ["capturedTurn", "capturedAt", "exportId"],
        properties: {
          capturedTurn: { type: "integer", minimum: 0 },
          capturedAt: { type: "string", format: "date-time" },
          exportId: { type: "string", minLength: 1 },
          scope: { type: "string", enum: ["own-only", "own-and-visible"] }
        }
      }
    },
    economy: {
      type: "object",
      additionalProperties: false,
      required: ["source", "visibility", "confidence"],
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { const: "own", type: "string" },
        confidence: { $ref: "#/$defs/confidenceKind" },
        sciencePerTurn: { type: "number" },
        culturePerTurn: { type: "number" },
        faithPerTurn: { type: "number" },
        goldPerTurn: { type: "number" },
        goldBalance: { type: "number" },
        faithBalance: { type: "number" },
        goldIncomePerTurn: { type: "number" },
        goldMaintenancePerTurn: { type: "number" }
      }
    }
  },
  $defs: {
    sourceKind: { type: "string", enum: ["lua-api", "ui-derived", "gameinfo-static", "inferred", "fixture"] },
    visibilityKind: { type: "string", enum: ["own", "visible-now", "revealed", "public-known", "player-visible"] },
    confidenceKind: { type: "string", enum: ["confirmed", "inferred", "low"] },
    namedType: {
      type: "object",
      required: ["type"],
      additionalProperties: false,
    ...
    playerSummary: {
      type: "object",
      required: ["source", "visibility", "confidence", "localPlayerId", "civilizationType", "leaderType"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
    ...
    city: {
      type: "object",
      required: ["source", "visibility", "confidence", "id", "name", "ownerPlayerId", "x", "y", "population"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
        ...
        currentProduction: { $ref: "#/$defs/namedType" },
        turnsUntilComplete: { type: "integer", minimum: 0 },
        yields: {
          type: "object",
        ...
        amenitiesNeeded: { type: "number" },
        turnsUntilGrowth: { type: "integer", minimum: 0 },
        turnsUntilStarvation: { type: "integer", minimum: 0 },
        buildings: { type: "array", items: { $ref: "#/$defs/namedType" } },
        districts: { type: "array", items: { $ref: "#/$defs/districtFact" } },
        populationLimitedDistrictsUsed: { type: "integer", minimum: 0 },
        populationLimitedDistrictsCapacity: { type: "integer", minimum: 0 },
        currentProductionProgress: { type: "number", minimum: 0 },
        currentProductionCost: { type: "number", minimum: 0 },
        foodStock: { type: "number", minimum: 0 },
        foodSurplus: { type: "number" },
        growthThreshold: { type: "number", minimum: 0 },
        underSiege: { type: "boolean" }
      }
    },
    unit: {
      type: "object",
      required: ["source", "visibility", "confidence", "id", "type", "ownerPlayerId", "x", "y"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
        ...
        movesRemaining: { type: "number", minimum: 0 },
        formationClass: { type: "string" },
        combatStrength: { type: "number", minimum: 0 },
        rangedStrength: { type: "number", minimum: 0 },
        bombardStrength: { type: "number", minimum: 0 },
        range: { type: "number", minimum: 0 },
        maxMoves: { type: "number", minimum: 0 },
        buildCharges: { type: "integer", minimum: 0 },
        experience: { type: "number", minimum: 0 },
        experienceForNextLevel: { type: "number", minimum: 0 },
        level: { type: "integer", minimum: 0 },
        promotions: { type: "array", items: { $ref: "#/$defs/namedType" } },
        militaryFormation: { type: "string", enum: ["standard", "corps", "army"] },
        upgradeCost: { type: "number", minimum: 0 }
      }
    },
    tile: {
      type: "object",
      required: ["source", "visibility", "confidence", "x", "y", "revealed", "visibleNow"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
        ...
        visibleNow: { type: "boolean" },
        terrainType: { type: "string" },
        featureType: { type: "string" },
        resourceType: { type: "string" },
        resourceAmount: { type: "integer", minimum: 0 },
        improvementType: { type: "string" },
        routeType: { type: "string" },
        districtType: { type: "string" },
        continentType: { type: "string" },
        isWater: { type: "boolean" },
        isLake: { type: "boolean" },
        isCoastalLand: { type: "boolean" },
        isFreshWater: { type: "boolean" },
        isRiver: { type: "boolean" },
        isHills: { type: "boolean" },
        isMountain: { type: "boolean" },
        isImpassable: { type: "boolean" },
        isNaturalWonder: { type: "boolean" },
        riverEdges: {
          type: "array",
          items: { type: "string", enum: ["W", "NW", "NE"] },
          uniqueItems: true
        },
        cliffEdges: {
          type: "array",
    ...
    progression: {
      type: "object",
      required: ["source", "visibility", "confidence", "current", "completed", "available"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { $ref: "#/$defs/visibilityKind" },
        confidence: { $ref: "#/$defs/confidenceKind" },
        current: { $ref: "#/$defs/namedType" },
        completed: {
          type: "array",
          items: { $ref: "#/$defs/namedType" }
        },
    ...
    availability: { type: "string", enum: ["available", "not-applicable", "unavailable"] },
    selection: {
      type: "object",
      required: ["source", "visibility", "confidence", "city", "unit"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { const: "own", type: "string" },
        confidence: { $ref: "#/$defs/confidenceKind" },
        city: { $ref: "#/$defs/selectionEntity" },
        unit: { $ref: "#/$defs/selectionEntity" }
      }
    },
    selectionEntity: {
      type: "object",
      ...
      oneOf: [
        ...
        {
          properties: {
            status: { enum: ["none", "unsupported", "error"] },
            id: { type: "null" }
          }
        }
      ],
      properties: {
        status: { type: "string", enum: ["selected", "none", "unsupported", "error"] },
        id: { type: ["string", "null"] }
      }
    },
    governors: {
      type: "object",
      required: ["source", "visibility", "confidence", "availability"],
      additionalProperties: false,
      properties: {
        source: { $ref: "#/$defs/sourceKind" },
        visibility: { const: "player-visible", type: "string" },
        confidence: { $ref: "#/$defs/confidenceKind" },
        availability: { $ref: "#/$defs/availability" },
        titlesAvailable: { type: "integer", minimum: 0 },
        titlesSpent: { type: "integer", minimum: 0 },
        canAppoint: { type: "boolean" },
        governors: {
          type: "array",
          items: {
            type: "object",
            ...
            properties: {
              ...
              assignedCityId: { type: "string" },
              assignedCityName: { type: "string" },
              established: { type: "boolean" },
              turnsToEstablish: { type: "integer", minimum: 0 },
              turnsUntilEstablished: { type: "integer", minimum: 0 },
              neutralizedTurns: { type: "integer", minimum: 0 },
              status: { type: "string", enum: ["needs-assignment", "transitioning", "established", "neutralized"] },
              promotions: { type: "array", items: { $ref: "#/$defs/namedType" } }
            }
          }
        }
        ...
        routes: {
          ...
          items: {
            ...
            properties: {
              ...
              turnsRemaining: { type: "integer", minimum: 0 }
            }
          }
        }
      }
    ...
    cityStates: {
      ...
      properties: {
        ...
        confidence: { $ref: "#/$defs/confidenceKind" },
        availability: { $ref: "#/$defs/availability" },
        availableEnvoys: { type: "integer", minimum: 0 },
        cityStates: {
          type: "array",
          items: {
            type: "object",
            ...
            properties: {
              ...
              rewards: {
                ...
                additionalProperties: false,
                properties: {
                  oneEnvoy: { type: "string" },
                  threeEnvoys: { type: "string" },
                  sixEnvoys: { type: "string" },
                  suzerain: { type: "string" }
                }
              },
              quests: {
                type: "array",
...
// tools/snapshot/src/fairness.ts
var forbiddenKeyPatterns = [
  /hidden/i,
...
function runFairnessChecks(snapshot) {
  const issues = [];
  ...
  for (const [index, value] of tiles.entries()) {
    ...
    if (tile.visibleNow !== true) {
      const allowed = /* @__PURE__ */ new Set(["source", "visibility", "confidence", "x", "y", "revealed", "visibleNow"]);
    ...
    if (tile.resourceAmount !== void 0 && typeof tile.resourceType !== "string") {
...
function walkForbiddenKeys(value, currentPath, issues) {
...
// tools/snapshot/src/module-cache.ts
var MODULE_FIELDS = {
  meta: void 0,
...
function moduleContractErrors(snapshot) {
  const errors = [];
  ...
  for (const moduleName of modules) {
    ...
    if (field && snapshot[field] === void 0) errors.push(`$.${field} is required for module ${moduleName}`);
  }
  ...
  if (modules.includes("government") && ["capturedTurn", "capturedAt", "exportId"].some((key) => status.government?.[key] !== status.policies?.[key])) {
...
async function validateSnapshotObject(snapshot) {
  const validate = await getValidator();
  const shapeOk = validate(snapshot);
  const schemaErrors = shapeOk ? [] : (validate.errors ?? []).map((error) => `${error.instancePath || "$"} ${error.message ?? "is invalid"}`);
  if (shapeOk) schemaErrors.push(...moduleContractErrors(snapshot));
...
async function getValidator() {
  ...
  addFormats(ajv);
  const compiled = ajv.compile(snapshot_schema_default);
  cachedValidator = compiled;
  return compiled;
}

// tools/render-map/src/render-map.ts
var RenderMapError = class extends Error {
...
async function renderSnapshotMapFile(snapshotPath, options2 = {}) {
  const snapshot = JSON.parse(await readFile(snapshotPath, "utf8"));
  return renderSnapshotMapObject(snapshot, options2);
}
async function renderSnapshotMapToFile(snapshotPath, outputPath, options2 = {}) {
  const rendered = await renderSnapshotMapFile(snapshotPath, options2);
  await writeFile(outputPath, rendered.svg, "utf8");
  return rendered;
}
async function renderSnapshotMapObject(snapshot, options2 = {}) {
  const validation = await validateSnapshotObject(snapshot);
  if (!validation.ok && !options2.allowInvalid) {
    throw new RenderMapError("snapshot failed schema or multiplayer fairness validation", validation);
  }
  const usable = (name) => snapshot.modules?.includes(name) && snapshot.moduleStatus?.[name]?.capturedTurn === snapshot.session?.gameTurn;
  const exportedTiles = usable("visibleMap") && Array.isArray(snapshot.visibleMap?.tiles) ? snapshot.visibleMap.tiles : [];
  const diskKeys = options2.disk ? new Set(options2.disk.map((coord) => coordKey(coord.x, coord.y))) : void 0;
  const tiles = diskKeys ? exportedTiles.filter((tile) => diskKeys.has(coordKey(tile.x, tile.y))) : exportedTiles;
  const cities = (usable("cities") && Array.isArray(snapshot.cities) ? snapshot.cities : []).filter((city) => !diskKeys || diskKeys.has(coordKey(city.x, city.y)));
  const units = (usable("units") && Array.isArray(snapshot.units) ? snapshot.units : []).filter((unit) => !diskKeys || diskKeys.has(coordKey(unit.x, unit.y)));
  const bounds = options2.bounds ?? getBounds(usable("visibleMap") ? snapshot.visibleMap?.bounds : void 0, tiles);
  const tileSize = options2.tileSize ?? defaultTileSize(options2.level);
  const layout = createHexLayout(bounds, tileSize);
  const padding = 24;
  const headerHeight = 54;
  const legendHeight = options2.level ? 78 : 140;
  const width = Math.max(options2.level === "world" ? 360 : 520, Math.ceil(layout.mapWidth + padding * 2));
  const height = Math.max(options2.level ? 180 : 260, Math.ceil(headerHeight + layout.mapHeight + padding + legendHeight));
  ...
  const missingCoords = (options2.disk ?? []).filter((coord) => !exportedTileKeys.has(coordKey(coord.x, coord.y)));
  const tileElements = tiles.map((tile) => renderTile(tile, bounds, layout, padding, headerHeight, options2.level)).join("\n");
  const coordinateOnlyTiles = renderCoordinateOnlyTiles({
    values: [...cities, ...units, ...missingCoords, ...options2.rings ?? []],
    exportedTileKeys,
    bounds,
    layout,
    padding,
  ...
  const ringElements = renderRings(options2.rings ?? [], bounds, layout, padding, headerHeight);
  const cityElements = cities.filter((city) => isWithinBounds(city, bounds)).map((city) => renderCity(city, bounds, layout, padding, headerHeight, options2.level)).join("\n");
  const unitElements = renderUnits({ tiles, unitsByCoord, unitsById, bounds, layout, padding, headerHeight });
  const tileLabels = options2.level === "world" ? "" : tiles.map((tile) => renderTileLabel(tile, cityById, bounds, layout, padding, headerHeight, options2.level)).join("\n");
  const title = `civ6-ai-copilot ${options2.level ?? "visible"} map turn ${snapshot.session?.gameTurn ?? "?"}`;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeXml(title)}">
  ...
  <text x="${padding}" y="42" font-family="Arial, sans-serif" font-size="11" fill="#52606d">visibility=${escapeXml(snapshot.source?.visibilityMode ?? "unknown")} \xB7 tiles=${tiles.length} \xB7 cities=${cities.length} \xB7 units=${units.length}</text>
  <g id="tiles">
...
function defaultTileSize(level) {
  if (level === "world") return 18;
  if (level === "region") return 28;
  if (level === "local") return 42;
  return 34;
}
function renderTile(tile, bounds, layout, padding, headerHeight, level) {
  ...
  const ownerRing = typeof tile.ownerPlayerId === "number" ? `<polygon points="${hexPoints(cx, cy, layout.radius * 0.78)}" fill="none" stroke="${ownerStroke(tile.ownerPlayerId)}" stroke-width="2" opacity="0.9"/>` : "";
...
function renderTileLabel(tile, cityById, bounds, layout, padding, headerHeight, level) {
  ...
  const labelText = label ? `<text x="${cx}" y="${cy - 2}" text-anchor="middle" font-family="Arial, sans-serif" font-size="8" font-weight="700" fill="#102a43">${escapeXml(label)}</text>` : "";
  const yieldText = yieldLine ? `<text x="${cx}" y="${cy + 9}" text-anchor="middle" font-family="Arial, sans-serif" font-size="7" fill="#102a43">${escapeXml(yieldLine)}</text>` : "";
...
function renderRings(rings, bounds, layout, padding, headerHeight) {
...
function renderCoordinateOnlyTiles(options2) {
  const seen = /* @__PURE__ */ new Set();
  const elements = [];
  for (const value of options2.values) {
    if (!isWithinBounds(value, options2.bounds)) {
      continue;
    }
    const key = coordKey(value.x, value.y);
    if (options2.exportedTileKeys.has(key) || seen.has(key)) {
      continue;
    }
    seen.add(key);
    const { cx, cy } = hexCenter(value, options2.bounds, options2.layout, options2.padding, options2.headerHeight);
    elements.push(
      `<g class="tile not-exported-coordinate" data-tile="${key}"><polygon points="${hexPoints(cx, cy, options2.layout.radius)}" fill="none" stroke="#6b7280" stroke-width="1.1" stroke-dasharray="3 3" opacity="0.78"><title>${escapeXml(`${key} \xB7 not in visibleMap.tiles`)}</title></polygon></g>`
    );
  }
  return elements.join("\n");
}
function renderCity(city, bounds, layout, padding, headerHeight, level) {
  const { cx, cy } = hexCenter(city, bounds, layout, padding, headerHeight);
  const caption = level === "region" && typeof city.population === "number" ? `${city.name ?? city.id ?? "City"} ${city.population}` : city.name ?? city.id ?? "City";
...
function renderUnits(options2) {
  const renderedUnitIds = /* @__PURE__ */ new Set();
  const elements = [];
  for (const tile of options2.tiles) {
    const tileUnits = [
      ...coordUnits(tile, options2.unitsByCoord),
      ...Array.isArray(tile.unitIds) ? tile.unitIds.map((unitId) => options2.unitsById.get(unitId)).filter(Boolean) : []
    ];
    for (const unit of tileUnits) {
      ...
      elements.push(renderUnit(unit, options2.bounds, options2.layout, options2.padding, options2.headerHeight));
    }
  }
  for (const unit of [...options2.unitsById.values()]) {
    if (renderedUnitIds.has(unit.id) || !isWithinBounds(unit, options2.bounds)) {
      continue;
    }
    renderedUnitIds.add(unit.id);
    elements.push(renderUnit(unit, options2.bounds, options2.layout, options2.padding, options2.headerHeight));
...
function renderUnit(unit, bounds, layout, padding, headerHeight) {
...
function getBounds(bounds, tiles) {
...
function createHexLayout(bounds, tileSize) {
...
function hexCenter(value, bounds, layout, padding, headerHeight) {
...
function groupUnitsByCoord(units) {
...
function coordUnits(tile, grouped) {
  return grouped.get(coordKey(tile.x, tile.y)) ?? [];
}
function coordKey(x, y) {
  return `${x ?? "?"},${y ?? "?"}`;
}
function terrainFill(terrainType, mountain = false) {
...
function ownerStroke(ownerPlayerId) {
...
function tileTitleText(tile, city) {
  ...
  const parts = [
    ...
    tile.featureType,
    tile.resourceType,
    tile.isFreshWater === true ? "fresh water" : void 0,
    tile.isRiver === true ? `river${tile.riverEdges && tile.riverEdges.length > 0 ? ` ${tile.riverEdges.join("/")}` : ""}` : void 0,
    tile.isCoastalLand === true ? "coastal land" : void 0,
    tile.isHills === true ? "hills" : void 0,
    tile.isMountain === true ? "mountain" : void 0,
    tile.isImpassable === true ? "impassable" : void 0,
    tile.cliffEdges && tile.cliffEdges.length > 0 ? `cliff ${tile.cliffEdges.join("/")}` : void 0,
    tile.improvementType,
    tile.routeType,
    tile.districtType,
    typeof tile.appeal === "number" ? `appeal ${tile.appeal}` : void 0,
    tile.yields ? `yields ${Object.entries(tile.yields).map(([key, value]) => `${key}=${value}`).join("/")}` : void 0
...
function resourceClass(resourceType) {
  if (!resourceType || resourceType === "UNKNOWN_RESOURCE" || resourceType === "-1") return void 0;
...
function readableName(value) {
  if (!value || value.startsWith("UNKNOWN")) return void 0;
...
var CHINESE_DISPLAY_NAME = projectVersion.chineseDisplayName;
var VERSION = projectVersion.version;
var COMPAT_VERSION = projectVersion.compatVersion;
var SCHEMA_VERSION = projectVersion.schemaVersion;
var PROTOCOL_VERSION = projectVersion.protocolVersion;
var WORKSHOP_VERSION = projectVersion.workshopVersion;
var MOD_ID = projectVersion.modId;
var MOD_GUID = projectVersion.modGuid;
var MOD_FOLDER_NAME_FROM_VERSION = projectVersion.modFolderName;
var SKILL_NAME = projectVersion.skillName;
var LUA_STATE_NAME = projectVersion.luaStateName;
var LOG_MARKER_PREFIX = projectVersion.logMarkerPrefix;
function compatFromVersion(version) {
  const match = version.match(/^(\d+)\.(\d+)\.\d+(?:[-+].*)?$/);
  return match ? `${match[1]}.${match[2]}` : void 0;
}

// tools/copilot/src/adjacent-units.ts
function adjacentOwnUnits(snapshot) {
  ...
  return units.filter((unit) => unit.ownerPlayerId === localPlayerId && Number.isInteger(unit.x) && Number.isInteger(unit.y)).map((unit) => ({
    id: text(unit.id),
    name: text(unit.name),
    type: text(unit.type),
    x: unit.x,
    y: unit.y,
    movesRemaining: Number.isInteger(unit.movesRemaining) ? unit.movesRemaining : void 0,
    adjacentTiles: screenAdjacentCoordinates(unit.x, unit.y).map(
      (neighbor) => describeAdjacentTile(neighbor, tileByCoord.get(`${neighbor.x},${neighbor.y}`))
    )
  }));
}
function screenAdjacentCoordinates(x, y) {
...
function describeAdjacentTile(neighbor, tile) {
  const view = { direction: neighbor.direction, x: neighbor.x, y: neighbor.y };
  if (!tile) return view;
  ...
  if (tile.yields && typeof tile.yields === "object") view.yields = tile.yields;
  return view;
}
function assignText(view, key, value) {
  if (typeof value === "string" && value.length > 0) view[key] = value;
}
function assignBoolean(view, key, value) {
...
// tools/copilot/src/decision-brief.ts
function buildDecisionBrief(snapshot) {
  const localPlayerId = snapshot.localPlayer?.localPlayerId;
  ...
  if (equipped.some((policy) => policy.slotType === void 0)) {
    gaps.push({
      kind: "not-collected",
      subject: "policySlots",
  ...
  return {
    brief: {
      player: {
        id: typeof localPlayerId === "number" ? localPlayerId : void 0,
        leader: text2(snapshot.localPlayer?.leaderType),
        civilization: text2(snapshot.localPlayer?.civilizationType)
      },
      counts: {
      ...
      policies: {
        government: text2(snapshot.government?.currentGovernment?.name) ?? text2(snapshot.government?.currentGovernment?.type),
        equipped,
...
function expandCity(snapshot, query) {
  const cities = arrayOf(snapshot.cities);
...
function expandUnit(snapshot, query) {
  const units = arrayOf(snapshot.units);
...
function cityRow(city) {
  return {
    ...
    housingTight: typeof city.housing === "number" && typeof city.population === "number" ? city.population > city.housing : void 0,
...
function domainGaps(snapshot) {
  const domains = [["governors", "\u603B\u7763"], ["trade", "\u5546\u8DEF"], ["cityStates", "\u57CE\u90A6\u5173\u7CFB"]];
  return domains.flatMap(([field, label]) => {
    const value = snapshot[field];
    if (!value || typeof value !== "object") {
      return [{ kind: "not-collected", subject: field, effect: `${label}\u672A\u91C7\u96C6\uFF0C\u4E0D\u80FD\u5F53\u6210\u6CA1\u6709\u6216\u5B89\u5168\u3002` }];
    }
    if (value.availability === "unavailable") {
      return [{ kind: "unavailable", subject: field, effect: `${label}\u8FD9\u6B21\u6CA1\u8BFB\u5230\uFF0C\u4E0D\u80FD\u636E\u6B64\u5224\u65AD\u6709\u65E0\u3001\u654C\u5BF9\u6216\u5B89\u5168\u3002` }];
    }
    if (value.availability === "not-applicable") {
      return [{ kind: "not-applicable", subject: field, effect: `${label}\u5728\u5F53\u524D\u89C4\u5219\u4E0B\u4E0D\u9002\u7528\u3002` }];
    }
    return [];
  });
}
...
function matchNamed(entities, value) {
...
// tools/copilot/src/map-view.ts
import { mkdir, writeFile as writeFile2 } from "node:fs/promises";
import path3 from "node:path";

// tools/render-map/src/map-png.ts
import { crc32, deflateSync } from "node:zlib";
function renderHexPng(options2) {
  const radius = options2.tileSize / 2;
  const hexWidth = Math.sqrt(3) * radius;
  const rowStep = radius * 1.5;
  const columns = options2.bounds.maxX - options2.bounds.minX + 1;
  const rows = options2.bounds.maxY - options2.bounds.minY + 1;
  ...
  for (const hex of options2.hexes) {
    const column = hex.x - options2.bounds.minX;
    const row = options2.bounds.maxY - hex.y;
...
function fillHex(rgb, width, height, cx, cy, radius, color, ring = false) {
...
// tools/copilot/src/map-view.ts
var MAX_MAP_RADIUS = 20;
var DEFAULT_MAP_RADIUS = { region: 8, local: 5 };
function parseMapSpec(spec) {
  ...
  if (level === "world") {
    ...
    const kind2 = marked ? parts[2] : parts[1];
    if (!isFocusKind(kind2)) return { error: `world \u6807\u8BB0\u65E0\u6CD5\u89E3\u6790\uFF1A${spec}` };
    return { level, mark: { kind: kind2, value: parts.slice(marked ? 3 : 2).join(":") } };
  }
  if (parts.length === 1) return { level };
  const kind = parts[1];
...
async function buildMapViews(snapshot, specs, outputDir) {
  const views = [];
  const gaps = [];
  ...
  for (const [index, spec] of specs.entries()) {
    ...
    const focus = parsed.level === "world" ? parsed.mark ? resolveFocus(snapshot, parsed.mark) : void 0 : resolveFocus(snapshot, parsed.focus ?? { kind: "selection", value: "" });
    if (focus && "error" in focus) {
      gaps.push(focus.error);
      continue;
    }
    const radiusResult = parsed.level === "world" ? void 0 : clampRadius(parsed.radius, parsed.level);
    if (radiusResult && radiusResult.note) gaps.push(radiusResult.note);
    const radius = radiusResult?.radius;
    const disk = focus && radius !== void 0 && "x" in focus ? coordsInRadius(focus, radius) : void 0;
    ...
    await writeFile2(svgPath, rendered.svg, "utf8");
...
function clampRadius(radius, level) {
...
function resolveFocus(snapshot, focus) {
  if (focus.kind === "coord") {
  ...
  if (focus.kind === "selection") {
    const selectedUnit = snapshot.selection?.unit?.status === "selected" ? snapshot.selection.unit.id : void 0;
    const selectedCity = snapshot.selection?.city?.status === "selected" ? snapshot.selection.city.id : void 0;
    const unit = selectedUnit ? arrayOf2(snapshot.units).find((item) => item.id === selectedUnit) : void 0;
    const city = selectedCity ? arrayOf2(snapshot.cities).find((item) => item.id === selectedCity) : void 0;
    const entity = unit ?? city;
    if (!entity || !Number.isInteger(entity.x) || !Number.isInteger(entity.y)) return { error: "\u5F53\u524D\u6CA1\u6709\u9009\u4E2D\u7684\u57CE\u5E02\u6216\u5355\u4F4D\u3002" };
    return { kind: unit ? "unit" : "city", id: text3(entity.id), name: text3(entity.name), x: entity.x, y: entity.y };
  }
  const entities = focus.kind === "city" ? arrayOf2(snapshot.cities) : arrayOf2(snapshot.units);
  const found = matchEntity(entities, focus.value);
  if ("error" in found) return found;
  return { kind: focus.kind, id: text3(found.entity.id), name: text3(found.entity.name), x: numberOf(found.entity.x), y: numberOf(found.entity.y) };
}
function matchEntity(entities, value) {
  const exactId = entities.find((entity) => entity.id === value);
...
function placesFor(snapshot, level, tiles, cities, units, focus) {
  ...
  if (level === "world") {
    places.wonders = tiles.filter((tile) => tile.isNaturalWonder === true).map((tile) => ({ name: readableName(text3(tile.featureType)), x: tile.x, y: tile.y }));
    return places;
  }
  places.tiles = tiles.map((tile) => compactTile(tile, level));
  if (level === "local") {
    const city = focus?.kind === "city" ? cities.find((item) => item.id === focus.id) ?? cities[0] : void 0;
    if (city) {
      places.cityCard = {
        name: city.name,
        population: city.population,
...
function compactTile(tile, level) {
  const compact = {
    x: tile.x,
    y: tile.y,
    terrain: readableName(text3(tile.terrainType)),
    feature: readableName(text3(tile.featureType)),
    resource: readableName(text3(tile.resourceType)),
    freshWater: tile.isFreshWater === true,
    river: tile.isRiver === true,
    hills: tile.isHills === true,
    mountain: tile.isMountain === true,
    improvement: readableName(text3(tile.improvementType)),
    route: readableName(text3(tile.routeType)),
    district: readableName(text3(tile.districtType)),
...
function pngHexes(tiles, cities, units, disk, rings) {
  ...
  return coords.map((coord) => {
    ...
    const resource = resourceClass(text3(tile?.resourceType));
...
function tilesIn(snapshot, disk) {
  const tiles = arrayOf2(snapshot.visibleMap?.tiles);
  if (!disk) return tiles;
  const keys = new Set(disk.map((coord) => `${coord.x},${coord.y}`));
  return tiles.filter((tile) => keys.has(`${tile.x},${tile.y}`));
}
function entitiesIn(entities, disk) {
  if (!disk) return entities;
  const keys = new Set(disk.map((coord) => `${coord.x},${coord.y}`));
  return entities.filter((entity) => keys.has(`${entity.x},${entity.y}`));
}
function exportedBounds(snapshot) {
  const bounds = snapshot.visibleMap?.bounds;
  if (!bounds || ![bounds.minX, bounds.maxX, bounds.minY, bounds.maxY].every((value) => Number.isInteger(value))) return void 0;
  return bounds;
}
...
// tools/copilot/src/prepare.ts
import { createHash as createHash2 } from "node:crypto";
import { mkdir as mkdir3, readFile as readFile3, rm } from "node:fs/promises";

// tools/bridge/src/bridge.ts
import { readFile as readFile2 } from "node:fs/promises";

// tools/bridge/src/parser.ts
import { TextDecoder } from "node:util";

// tools/bridge/src/protocol.ts
var SNAPSHOT_BEGIN = `${LOG_MARKER_PREFIX}_SNAPSHOT_BEGIN`;
var SNAPSHOT_CHUNK = `${LOG_MARKER_PREFIX}_SNAPSHOT_CHUNK`;
var SNAPSHOT_END = `${LOG_MARKER_PREFIX}_SNAPSHOT_END`;
var COPILOT_LOADED = `${LOG_MARKER_PREFIX}_LOADED`;
var COPILOT_DIAGNOSTIC = `${LOG_MARKER_PREFIX}_DIAGNOSTIC`;

// tools/bridge/src/parser.ts
var BridgeParseError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "BridgeParseError";
  }
};
var markers = [SNAPSHOT_BEGIN, SNAPSHOT_CHUNK, SNAPSHOT_END];
function parseLogContent(content) {
  const exportsById = /* @__PURE__ */ new Map();
  for (const [lineOffset, line] of content.split(/\r?\n/).entries()) {
    ...
    if (marker === SNAPSHOT_CHUNK) {
      const chunk2 = assertChunk(payload, lineNumber);
      const parsed2 = ensureExport(exportsById, chunk2.exportId, lineNumber);
      if (parsed2.end) parsed2.issues.push(`line ${lineNumber}: chunk appeared after end for exportId ${chunk2.exportId}`);
      parsed2.chunks.push(chunk2);
      parsed2.lineNumbers.push(lineNumber);
      continue;
    }
...
function diagnoseLogContent(content) {
  const loadedLines = [];
  const diagnostics = [];
  const issues = [];
  let parsedExports = [];
    ...
    if (line.includes(COPILOT_DIAGNOSTIC)) {
      try {
        ...
        if (payload && typeof payload === "object" && !Array.isArray(payload)) {
          diagnostics.push({ lineNumber, payload });
        } else {
...
function assembleLatestCompleteExport(exports) {
  const complete = exports.filter((candidate) => candidate.end).sort((a, b) => Math.max(...a.lineNumbers) - Math.max(...b.lineNumbers));
...
function assembleExport(parsed) {
  const { begin } = parsed;
  if (!parsed.end) {
  ...
  const payloadBase64 = [...parsed.chunks].sort((a, b) => a.index - b.index).map((chunk2) => chunk2.data).join("");
  ...
  try {
    ...
    throw new BridgeParseError(`Export ${begin.exportId} decoded payload is not valid UTF-8: ${error.message}`);
  }
  let snapshot;
  try {
    ...
    throw new BridgeParseError(
      `Export ${begin.exportId} decoded payload is not valid JSON: ${error.message}`
    );
  }
...
function parseJsonAfterMarker(line, marker, lineNumber) {
  ...
  try {
    ...
    throw new BridgeParseError(`line ${lineNumber}: invalid ${marker} JSON payload: ${error.message}`);
  }
}
function ensureExport(exportsById, exportId, lineNumber) {
...
function assertBegin(value, lineNumber) {
  const payload = assertObject(value, lineNumber, SNAPSHOT_BEGIN);
  const begin = payload;
  if (begin.protocolVersion !== PROTOCOL_VERSION || typeof begin.exportId !== "string" || begin.exportId.length === 0 || typeof begin.schemaVersion !== "string" || begin.schemaVersion.length === 0 || !Number.isSafeInteger(begin.chunkCount) || begin.chunkCount <= 0 || !Number.isSafeInteger(begin.byteLength) || begin.byteLength <= 0 || begin.encoding !== "base64-json") {
    throw new BridgeParseError(`line ${lineNumber}: invalid ${SNAPSHOT_BEGIN} payload shape.`);
  }
  return begin;
}
function assertChunk(value, lineNumber) {
  const payload = assertObject(value, lineNumber, SNAPSHOT_CHUNK);
  const chunk2 = payload;
  if (typeof chunk2.exportId !== "string" || chunk2.exportId.length === 0 || !Number.isSafeInteger(chunk2.index) || chunk2.index < 0 || typeof chunk2.data !== "string") {
    throw new BridgeParseError(`line ${lineNumber}: invalid ${SNAPSHOT_CHUNK} payload shape.`);
  }
  return chunk2;
}
function assertEnd(value, lineNumber) {
...
function assertObject(value, lineNumber, marker) {
...
function isStructurallyComplete(parsed) {
  return Boolean(parsed.end) && parsed.issues.length === 0 && getChunkStructureIssue(parsed) === void 0;
}
function getChunkStructureIssue(parsed) {
  ...
  for (const chunk2 of chunks) {
    if (chunk2.index < 0 || chunk2.index >= begin.chunkCount) {
      return `Export ${begin.exportId} has out-of-range chunk index ${chunk2.index}.`;
    }
    if (seen.has(chunk2.index)) {
      return `Export ${begin.exportId} has duplicate chunk index ${chunk2.index}.`;
    }
    seen.add(chunk2.index);
  }
  return void 0;
}

// tools/bridge/src/writer.ts
import { createHash } from "node:crypto";
import { mkdir as mkdir2, rename, writeFile as writeFile3 } from "node:fs/promises";
import path4 from "node:path";
async function writeSnapshotOutputs(snapshot, outputDir, metadata) {
  ...
  const localPlayerId = Number.isInteger(typed.localPlayer?.localPlayerId) ? String(typed.localPlayer?.localPlayerId) : "unknown-player";
  const exportId = sanitizePathPart(metadata.exportId);
  const sessionDir = path4.join(outputDir, sessionId);
  const snapshotPath = path4.join(sessionDir, `turn-${gameTurn}-player-${localPlayerId}-${exportId}.snapshot.json`);
  const latestPath = path4.join(outputDir, "latest.json");
  const manifestPath = path4.join(outputDir, "latest-manifest.json");
...
async function atomicWrite(targetPath, content) {
  ...
  await writeFile3(tempPath, content, "utf8");
...
// tools/bridge/src/bridge.ts
async function runBridgeOnce(options2) {
  let diagnostics;
  try {
    const logContent = await readFile2(options2.inputLog, "utf8");
    diagnostics = diagnoseLogContent(logContent);
    if (options2.diagnoseOnly) {
      return {
    ...
    if (options2.skipExportId && assembled.exportId === options2.skipExportId) {
      return {
        ok: true,
    ...
    const written = await writeSnapshotOutputs(assembled.snapshot, options2.outputDir, {
      exportId: assembled.exportId
    });
    return {
      ok: validation.ok,
      ...
      written,
      diagnostics,
      exitCode: validation.ok || options2.allowInvalid ? 0 : 2
    };
  } catch (error) {
    return {
      ok: false,
...
// tools/tuner-bridge/src/nexus-client.ts
import net from "node:net";
import { randomUUID } from "node:crypto";
var TAG_HANDSHAKE = 4;
var TAG_COMMAND = 3;
var TAG_OUTPUT = 4294967295;
var TunerProtocolError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "TunerProtocolError";
  }
};
var TunerClient = class _TunerClient {
  ...
  static async connect(options2 = {}) {
    const host = options2.host ?? "127.0.0.1";
    const ports = options2.ports?.length ? options2.ports : [4318, 4319];
    const timeoutMs = options2.timeoutMs ?? 8e3;
    const appName = options2.appName ?? "civ6-ai-copilot";
    const errors = [];
    for (const port of ports) {
      ...
      try {
        ...
        const client = new _TunerClient(socket, { host, port, timeoutMs });
...
print("${sentinel}")`);
    const output = [];
    while (true) {
      const frame = await this.readFrame(options2.timeoutMs);
      if (frame.tag !== TAG_OUTPUT) {
        continue;
      }
      const text5 = decodeOutput(frame.payload);
      const sentinelIndex = text5.indexOf(sentinel);
      if (sentinelIndex >= 0) {
        const beforeSentinel = text5.slice(0, sentinelIndex);
  ...
  async sendFrame(tag, text5) {
    const payload = Buffer.from(`${text5}\0`, "utf8");
  ...
  async readFrame(timeoutMs = this.timeoutMs) {
    while (true) {
...
function chooseDefaultState(states) {
...
function parseStateList(payload) {
  const tokens = payload.toString("utf8").split("\0").filter((token) => token.length > 0);
...
// tools/tuner-bridge/src/tuner-bridge.ts
var TUNER_ERROR_MARKER = "CIV6_AI_COPILOT_TUNER_ERROR";
var READ_CACHED_EXPORT_LUA = `
...
`;
async function runTunerBridgeOnce(options2) {
  let client;
  let tuner;
  let diagnostics;
  try {
    client = await TunerClient.connect({
      host: options2.host,
      ports: options2.ports,
      timeoutMs: options2.timeoutMs,
      appName: "civ6-ai-copilot"
    });
    const state = options2.state ?? chooseDefaultState(client.states);
    if (!state) {
...
function extractTunerError(output) {
...
// tools/copilot/src/prepare.ts
async function runCopilotRefresh(paths, options2) {
  const requestedMode = options2.refreshMode ?? "auto";
  ...
  if (mode === "tuner") {
    const result2 = await runTunerBridgeOnce({
      outputDir: paths.snapshotDir,
      host: options2.host,
      ports: options2.port ? [options2.port] : void 0,
      state: options2.state ?? LUA_STATE_NAME,
      timeoutMs: options2.timeoutMs,
      allowInvalid: options2.allowInvalid,
    ...
    return {
      ...
      summary: summarizeRefreshResult(result2),
      result: result2
    };
  }
...
function resolveRefreshMode(mode, platform) {
  if (mode !== "auto") {
    return mode;
  }
...
async function readCurrentExportId(snapshotDir) {
  try {
    const [manifestText, latestText] = await Promise.all([
      readFile3(`${snapshotDir}/latest-manifest.json`, "utf8"),
      readFile3(`${snapshotDir}/latest.json`, "utf8")
    ]);
    const manifest = JSON.parse(manifestText);
    const latest = JSON.parse(latestText);
    const exportId = typeof manifest.exportId === "string" ? manifest.exportId : void 0;
    if (!exportId || latest.source?.exportId !== exportId) return void 0;
    if (typeof manifest.checksumSha256 === "string") {
      const checksum = createHash2("sha256").update(Buffer.from(latestText, "utf8")).digest("hex");
      if (checksum !== manifest.checksumSha256) return void 0;
    }
    return exportId;
  } catch {
    return void 0;
...
var KNOWN_MODULES = Object.keys(MODULE_TO_KEYS);
async function runCopilotContext(options2 = {}) {
  const modules = normalizeModules(options2.modules);
  const paths = buildCiv6AICopilotPaths({
    platform: options2.platform,
    homeDir: options2.homeDir,
    civ6UserDataDir: options2.civ6UserDataDir,
    modsDir: options2.modsDir,
    logsDir: options2.logsDir,
    luaLogPath: options2.luaLogPath,
    codexHome: options2.codexHome,
    snapshotDir: options2.snapshotDir,
    question: options2.question,
    requiredModules: modules
  });
  await mkdir4(paths.snapshotDir, { recursive: true });
  const refresh = await runCopilotRefresh(paths, options2);
  if (refresh.attempted && !refresh.ok) {
    const status = noCompletedExport(refreshText(refresh)) ? "needs-game-refresh" : "runtime-error";
    return incomplete(status, options2.question, modules, refresh, [refreshText(refresh)], refreshActions(status, refresh, paths));
  }
  const latestPath = path5.join(paths.snapshotDir, "latest.json");
  let snapshotText = "";
  try {
    snapshotText = await readFile4(latestPath, "utf8");
  } catch {
    return incomplete(
      "needs-game-refresh",
      options2.question,
  ...
  try {
    ...
    return incomplete("runtime-error", options2.question, modules, refresh, [`latest.json \u4E0D\u662F\u6709\u6548 JSON\uFF1A${error.message}`], [
      "\u91CD\u65B0\u70B9\u51FB\u300C\u66F4\u65B0\u6218\u60C5\u300D\uFF0C\u5F85\u9762\u677F\u663E\u793A\u5DF2\u6C47\u603B\u540E\u518D\u83B7\u53D6\u3002"
    ]);
  }
  const validation = await validateSnapshotObject(snapshot);
  if (!validation.ok) {
    return incomplete("runtime-error", options2.question, modules, refresh, [
  ...
  if (compatIssue) {
    return incomplete("runtime-error", options2.question, modules, refresh, [compatIssue], [
  ...
  if (manifest.issues.length > 0) {
    return incomplete("runtime-error", options2.question, modules, refresh, manifest.issues, [
      "\u5F53\u524D\u6218\u60C5\u6587\u4EF6\u548C manifest \u4E0D\u4E00\u81F4\u3002\u91CD\u65B0\u70B9\u51FB\u300C\u66F4\u65B0\u6218\u60C5\u300D\u540E\u518D\u83B7\u53D6\u3002"
    ]);
  }
  const mapSpecs = options2.mapSpecs ?? [];
  ...
  const detail = {};
  if (options2.city) {
    const expanded = expandCity(snapshot, options2.city);
    if (expanded.gap) gaps.push(expanded.gap);
    if (expanded.detail) detail.city = expanded.detail;
  }
  if (options2.unit) {
    const expanded = expandUnit(snapshot, options2.unit);
    if (expanded.gap) gaps.push(expanded.gap);
    if (expanded.detail) detail.unit = expanded.detail;
  }
  if (options2.adjacentUnits) {
    detail.adjacentUnits = adjacentOwnUnits(snapshot);
  }
  let mapViews;
  if (mapSpecs.length > 0) {
    const exportId = text4(snapshot.source?.exportId) ?? "current";
    const built = await buildMapViews(snapshot, mapSpecs, path5.join(paths.snapshotDir, "map-views", exportId.replace(/[^\w.-]+/g, "_")));
    mapViews = built.views;
    gaps.push(...built.gaps.map((effect) => ({ kind: "partial", subject: "map", effect })));
  }
  let artifacts;
  const warnings = [...manifest.warnings];
  if (options2.raw) {
    const exportId = (text4(snapshot.source?.exportId) ?? "current").replace(/[^\w.-]+/g, "_");
    const rawPath = path5.join(paths.snapshotDir, "raw", `${exportId}.json`);
    await mkdir4(path5.dirname(rawPath), { recursive: true });
    await writeFile4(rawPath, snapshotText, "utf8");
    artifacts = { raw: { path: rawPath } };
  }
  if (options2.renderMapPath) {
    try {
      await mkdir4(path5.dirname(options2.renderMapPath), { recursive: true });
      const rendered = await renderSnapshotMapToFile(latestPath, options2.renderMapPath);
      artifacts = { ...artifacts, visibleMap: { path: options2.renderMapPath, tiles: rendered.counts.tiles } };
    } catch (error) {
      warnings.push(`visible-map.svg \u6E32\u67D3\u5931\u8D25\uFF1A${error.message}`);
    }
  }
  return {
    contractVersion: "1",
...
function modulesInSnapshot(snapshot) {
  const declared = Array.isArray(snapshot.modules) ? snapshot.modules.filter((name) => typeof name === "string") : [];
...
function moduleInExport(snapshot, moduleName) {
  if (Array.isArray(snapshot.modules) && snapshot.modules.includes(moduleName)) return true;
  return (MODULE_TO_KEYS[moduleName] ?? []).some((key) => snapshot[key] !== void 0);
}
function incomplete(status, query, modules, refresh, issues, userActions) {
  return {
    contractVersion: "1",
    status,
...
function identityOf(snapshot) {
  return {
    exportId: text4(snapshot.source?.exportId),
    sessionId: text4(snapshot.session?.sessionId),
    gameTurn: Number.isInteger(snapshot.session?.gameTurn) ? snapshot.session.gameTurn : void 0,
    exportedAt: text4(snapshot.exportedAt),
    modVersion: text4(snapshot.source?.modVersion),
    compatVersion: text4(snapshot.source?.compatVersion),
    protocolVersion: text4(snapshot.source?.protocolVersion),
    schemaVersion: text4(snapshot.schemaVersion)
  };
}
function incompatibleCompat(snapshot) {
  const declared = text4(snapshot.source?.compatVersion);
  const inferred = text4(snapshot.source?.modVersion) ? compatFromVersion(String(snapshot.source.modVersion)) : void 0;
...
async function readManifest(snapshotDir, snapshotText, snapshot) {
  const manifestPath = path5.join(snapshotDir, "latest-manifest.json");
  let manifestText = "";
  try {
  ...
  try {
    ...
    return { issues: [`latest-manifest.json \u4E0D\u662F\u6709\u6548 JSON\uFF1A${error.message}`], warnings: [] };
  }
  const issues = [];
  const checksum = createHash3("sha256").update(Buffer.from(snapshotText, "utf8")).digest("hex");
  ...
  if (typeof manifest.exportId === "string" && text4(snapshot.source?.exportId) && manifest.exportId !== snapshot.source.exportId) {
...
function compactRefresh(refresh) {
  ...
  return {
    ...
    exportId: result && "exportId" in result ? result.exportId : void 0
  };
}
...
function noCompletedExport(text5) {
  return /no-cached-export|No complete civ6-ai-copilot snapshot|没有找到 snapshot/i.test(text5);
}
function refreshActions(status, refresh, paths) {
...
// tools/copilot/src/context-cli.ts
program.name("civ6-ai-copilot-context").description("Return one canonical, validated Civ6 context payload for Agent analysis.").option("--query <text>", "The player's question. Stored for the agent; it does not select modules.").option("--module <module>", "Ignored for output. Raw modules are not inlined; use --city, --unit, --map, or --raw.", collectList, []).option("--city <name>", "Expand one city by name or id.").option("--unit <name>", "Expand one unit by name or id.").option("--raw", "Write the raw export to a file and return its path only.", false).option("--adjacent-units", "Add odd-r adjacent tiles for the local player's units.", false).option("--render-map <path>", "Write a player-visible hex SVG for this export.").option("--map <spec>", "Map view. Repeat. world | region[:city|unit|coord|selection:value[:radius]] | local[:...]. Region defaults to radius 8, local to 5, maximum 20.", collectList, []).option("--platform <platform>", "Target platform: win32, darwin, or linux. Defaults to the current platform.").option("--home <dir>", "Target user's home directory. Defaults to the current user's home.").option("--civ6-user-data-dir <dir>", "Override the Civ6 user-data root.").option("--mods-dir <dir>", "Override the Civ6 Mods directory.").option("--logs-dir <dir>", "Override the Civ6 Logs directory.").option("--lua-log <path>", "Override the full Civilization VI Lua.log path.").option("--codex-home <dir>", "Codex home directory. Defaults to <home>/.codex.").option("--snapshot-dir <dir>", "Snapshot output directory.").option("--refresh <mode>", "Snapshot refresh mode: auto, tuner, bridge, or none.", "auto").option("--host <host>", "Tuner host.", "127.0.0.1").option("--port <port>", "Specific tuner port; defaults to trying 4318 then 4319.", parsePort).option("--state <name>", "Lua state to read from; defaults to project luaStateName.").option("--timeout-ms <ms>", "Socket read/write timeout.", parsePositiveInteger, 8e3).option("--allow-invalid", "Allow refresh to write an invalid snapshot for diagnostics.", false).addOption(new Option("--question <text>", "Legacy alias for --query.").hideHelp()).parse();
var options = program.opts();
if (!isRefreshMode(options.refresh)) {
  program.error("--refresh must be one of: auto, tuner, bridge, none");
}
var report = await runCopilotContext({
